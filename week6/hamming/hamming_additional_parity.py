...
data_indexes = [i for i in range(0, n-1) if i not in parity_indexes]
...
def encode(message):
    ...
    # 3. Block parity bit
    count = sum(i == "1" for i in codeword[:-1])
    codeword[n-1] = "1" if count % 2 == 1 else "0"
...
def decode(codeword):
    ...
    # 1. Block wide parity check
    count = sum(i == "1" for i in codeword)
    block_parity_check = count % 2 == 0
    ...
    # We can detect 2 errors
    if block_parity_check and failed_parity_checks > 0:
        print(colored(255, 0, 0, f"DETECTED ERROR"))
        return ""

    # We can correct 1 error
    if not block_parity_check and failed_parity_checks > 0:
